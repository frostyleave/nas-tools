...
class BrushTaskV2(object):
    ...
    def init_config(self):
        ...
        # 启动任务
        self._scheduler = BackgroundScheduler(timezone=Config().get_timezone(), executors={"default": ThreadPoolExecutor(5)})
        scheduler_jobs = []
        for _, task in self._brush_tasks.items():
            ...
            if task_state == 'Y' and task.get("interval"):
                ...
                if cron.isdigit():
                    job_item = self._scheduler.add_job(func=self.check_task_rss,
                                                       args=[task.get("id")],
                                                       trigger='interval',
                                                       seconds=int(cron) * 60,
                                                       name='[刷流]' + task.get("name"))
                    if job_item:
                        scheduler_jobs.append(job_item)
                elif cron.count(" ") == 4:
                    try:
                        job_item = self._scheduler.add_job(func=self.check_task_rss,
                                                           args=[task.get("id")],
    ...
    def load_brushtasks(self):
        ...
        for task in brushtasks:
            site_info = self.sites.get_site(siteid=task.SITE)
            if not site_info:
                continue
            ...
            self._brush_tasks[str(task.ID)] = {
                ...
                "rss_url": task.RSSURL if task.RSSURL else site_info.rssurl,
    ...
    def check_task_rss(self, taskid):
        ...
        site_info = self.sites.get_site(siteid=site_id)
        if not site_info:
